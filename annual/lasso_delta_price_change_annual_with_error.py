...
def run_delta_models_annual_with_error(df, start_from='2020'):
    """
    Run delta models for annual data with error feature from previous year pair
    Sequentially trains models: for year pair (t, t+1), uses error from pair (t-1, t) as feature
    """
    # First aggregate quarterly data to annual
    ...
    # Store errors from previous year pairs: {year_pair: {product_idx: error}}
    errors = {}  # Key: (year_prev, year_curr), Value: {product_idx: error}

    for i in range(len(years) - 1):
        year_prev, year_curr = years[i], years[i + 1]
        period_pair = (year_prev, year_curr)
        ...
        # Add error feature from previous year pair if available
        if i == 0:
            # First year pair: no error feature, use base features only
            X = X_base
            use_error_feature = False
        else:
            # Get previous year pair
            prev_year_prev, prev_year_curr = years[i-1], years[i]
            prev_period_pair = (prev_year_prev, prev_year_curr)
        ...
        # Store errors for next year pair (only for products with actual prices)
        errors[period_pair] = {}
...
def main():
    ...
    print('Running annual delta models with error feature...')
    model_df, coef_df, delta_df, df_processed, years = run_delta_models_annual_with_error(df, start_from='2020')
    ...
    out = 'Lasso_Delta_Models_Annual_With_Error.xlsx'
    ...
    print(f'\nWrote {out}')
    print('\n=== Annual Delta Model with Error Feature Summary ===')
