...
def run_delta_models_annual(df, start_from='2020'):
    """
    Run OLS Delta models for annual data
    Directly models log price differences between adjacent years using OLS
    """
    # First aggregate quarterly data to annual
    ...
    for i in range(len(years) - 1):
        ...
        # Use OLS instead of Lasso
        ols = LinearRegression()
        ols.fit(X_scaled, y)
        
        r2 = ols.score(X_scaled, y)
        n_features = len(feature_cols)
        
        # In-sample predictions (for training data)
        y_hat_in = ols.predict(X_scaled)
        jevons_actual_train = float(np.mean(y))
        jevons_pred_in = float(np.mean(y_hat_in))
        ...
        log_deltas_predicted = ols.predict(X_predict_scaled)
        ...
        model_summary_rows.append({
            ...
            'Delta_Actual_%': float(np.nanmean(log_deltas_actual)) * 100.0 if np.any(has_actual) else np.nan,
            'Delta_Predicted_%': float(np.nanmean(log_deltas_predicted)) * 100.0,
        })
        
        for f, c in zip(feature_cols, ols.coef_):
            coef_rows.append({
                'Base Year': year_prev,
                'Current Year': year_curr,
                'Feature': f,
...
def main():
    ...
    print('Running annual OLS Delta models...')
    ...
    out = 'OLS_Delta_Models_Annual.xlsx'
    ...
    print(f'\nWrote {out}')
    print('\n=== Annual OLS Delta Model Summary ===')
